...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let keypair = ml_dsa_65::generate_key_pair(keygen_randomness);
    let sk = keypair.signing_key;
    let pk = keypair.verification_key;

    let message = b"";
    let sig = ml_dsa_65::sign(&sk, message, context, signing_randomness)
        .map_err(|e| format!("signing failed: {:?}", e))?;

    write("/tmp/ml-dsa-65-pub-key", pk.as_slice())?;
    write("/tmp/ml-dsa-65-sig", sig.as_slice())?;
