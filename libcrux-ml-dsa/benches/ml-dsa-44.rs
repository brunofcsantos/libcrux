...
pub fn comparisons_key_generation(c: &mut Criterion<CpuCycles>) {
    ...
    group.bench_function("libcrux (external random)", move |b| {
        b.iter(|| {
            let _ = ml_dsa_44::generate_key_pair(randomness);
        });
    });
    ...
    group.bench_function("pqclean (internal random)", |b| {
        b.iter(|| {
            let _ = pqcrypto_mldsa::mldsa44::keypair();
        });
    });
...
pub fn comparisons_signing(c: &mut Criterion<CpuCycles>) {
    let mut rng = OsRng;
    let mut group = c.benchmark_group("ML-DSA-44 Signing");
    group.measurement_time(Duration::from_secs(10));
    ...
    group.bench_function("libcrux (external random)", move |b| {
        b.iter(|| {
            let _ = ml_dsa_44::sign(&keypair.signing_key, &message, b"", randomness);
        })
    });

    #[cfg(not(all(target_os = "macos", target_arch = "x86_64")))]
    {
        let (_, sk) = pqcrypto_mldsa::mldsa44::keypair();
        group.bench_function("pqclean (internal random)", move |b| {
            b.iter(|| {
                let _ = pqcrypto_mldsa::mldsa44::detached_sign(&message, &sk);
            })
        });
    }
}
...
pub fn comparisons_verification(c: &mut Criterion<CpuCycles>) {
    let mut rng = OsRng;
    let mut group = c.benchmark_group("ML-DSA-44 Verification");
    group.measurement_time(Duration::from_secs(10));
    ...
    rng.try_fill_bytes(&mut randomness).unwrap();
    let signature = ml_dsa_44::sign(&keypair.signing_key, &message, b"", randomness).unwrap();

    group.bench_function("libcrux", move |b| {
        b.iter(|| {
            let _ =
                ml_dsa_44::verify(&keypair.verification_key, &message, b"", &signature).unwrap();
        })
    });

    #[cfg(not(all(target_os = "macos", target_arch = "x86_64")))]
    {
        let (vk, sk) = pqcrypto_mldsa::mldsa44::keypair();
        let signature = pqcrypto_mldsa::mldsa44::detached_sign(&message, &sk);

        group.bench_function("pqclean", move |b| {
            b.iter(|| {
                let _ =
                    pqcrypto_mldsa::mldsa44::verify_detached_signature(&signature, &message, &vk)
                        .unwrap();
            })
        });
    }
