...
use libcrux_p256::{ecdsa_sign_p256_sha2, ecdsa_verif_p256_sha2};
...
fn bench_ecdsa_verify_cycles(c: &mut Criterion<CpuCycles>) {
    ...
    let mut group = c.benchmark_group("ecdsa_verif_p256_sha2");
    group.throughput(Throughput::Bytes(msg_len as u64));
    group.measurement_time(Duration::from_secs(10));

    group.bench_function("ecdsa_verif_p256_sha2", |b| {
        b.iter(|| {
            let _valid = ecdsa_verif_p256_sha2(
                black_box(msg_len),
                black_box(&message),
